...
Namespace Common
	...
	Public Class BinaryExperimentProgressHandler
        ...
        Public Sub Report(iterationResult As RunDetail(Of BinaryClassificationMetrics)) Implements IProgress(Of RunDetail(Of BinaryClassificationMetrics)).Report
	...
	Public Class MulticlassExperimentProgressHandler
        ...
        Public Sub Report(iterationResult As RunDetail(Of MulticlassClassificationMetrics)) Implements IProgress(Of RunDetail(Of MulticlassClassificationMetrics)).Report
            'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
            'ORIGINAL LINE: if (_iterationIndex++ == 0)
            If _iterationIndex = 0 Then
                _iterationIndex += 1
	...
	Public Class RegressionExperimentProgressHandler
        ...
        Public Sub Report(iterationResult As RunDetail(Of RegressionMetrics)) Implements IProgress(Of RunDetail(Of RegressionMetrics)).Report
            'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
            'ORIGINAL LINE: if (_iterationIndex++ == 0)
            If _iterationIndex = 0 Then
                _iterationIndex += 1

...
Namespace Common
	Public Class ObjectPool(Of T)
		...
		Private _objects As ConcurrentBag(Of Tuple(Of T, Date))
        ...
        Public Sub New(objectGenerator As Func(Of T), Optional minPoolSize As Integer = 5,
                       Optional maxPoolSize As Integer = 1000, Optional expirationTime As Double = 30000)
            ...
            _objects = New ConcurrentBag(Of Tuple(Of T, Date))()
            ...
            For i As Integer = 0 To minPoolSize - 1
                Dim tuple As New Tuple(Of T, Date)(_objectGenerator(), Date.UtcNow)
                _objects.Add(tuple)
            Next i
        ...
        Public Function GetObject() As T
            ...
            Do While Not _objects.IsEmpty
                Dim tuple As Tuple(Of T, Date) = Nothing
                If _objects.TryTake(tuple) Then
                    If Date.UtcNow.Subtract(tuple.Item2) < TimeSpan.FromMilliseconds(_expirationTime) Then
        ...
        Public Sub PutObject(item As T)
            ...
            If _objects.Count < _maxPoolSize Then
                ...
                Dim tuple As New Tuple(Of T, Date)(item, Date.UtcNow)
                _objects.Add(tuple)
            End If
        End Sub
    End Class

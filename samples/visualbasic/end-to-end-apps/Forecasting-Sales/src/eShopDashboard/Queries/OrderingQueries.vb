...
'INSTANT VB NOTE: In the following line, Instant VB substituted 'Object' for 'dynamic' - this will work in VB with Option Strict Off:
		...
		Public Async Function GetProductDataAsync(productId As String) As Task(Of IEnumerable(Of ProductData)) Implements IOrderingQueries.GetProductDataAsync
			Using connection = New SqlConnection(_connectionString)
				connection.Open()

				Dim productHistory = Await connection.QueryAsync(Of ProductData)(OrderingQueriesText.ProductHistory(productId), New With {Key productId})

				Return productHistory.Where(Function(p) p.next <> 0 AndAlso p.prev <> 0)
			End Using
		End Function
	End Class
End Namespace
...
Namespace CommonHelpers
	Public Class MLModelEngine(Of TData As Class, TPrediction As {Class, New})
		...
		Public Sub New(mlContext As MLContext, modelFilePathName As String, Optional minPredictionEngineObjectsInPool As Integer = 5, Optional maxPredictionEngineObjectsInPool As Integer = 1000, Optional expirationTime As Double = 30000)
			_mlContext = mlContext

			'Load the ProductSalesForecast model from the .ZIP file
			Dim modelInputSchema As Object
			_model = mlContext.Model.Load(modelFilePathName, modelInputSchema)
		...
		''' <param name="maxPredictionEngineObjectsInPool">Maximum number of PredictionEngineObjects in pool</param>
		''' <param name="expirationTime">Expiration Time (mlSecs) of PredictionEngineObject since added to the pool</param>
		Public Sub New(mlContext As MLContext, model As ITransformer, Optional minPredictionEngineObjectsInPool As Integer = 5, Optional maxPredictionEngineObjectsInPool As Integer = 1000, Optional expirationTime As Double = 30000)
			_mlContext = mlContext
			_model = model
			_minPredictionEngineObjectsInPool = minPredictionEngineObjectsInPool
			_maxPredictionEngineObjectsInPool = maxPredictionEngineObjectsInPool
		...
		Private Function CreatePredictionEngineObjectPool() As ObjectPool(Of PredictionEngine(Of TData, TPrediction))
			Return New ObjectPool(Of PredictionEngine(Of TData, TPrediction))(objectGenerator:=Function()
																				'Measure PredictionEngine creation
																				Dim watch = System.Diagnostics.Stopwatch.StartNew()

																				'Make PredictionEngine
																				Dim predEngine = _mlContext.Model.CreatePredictionEngine(Of TData, TPrediction)(_model)
																				...
																				Return predEngine
			End Function, minPoolSize:= _minPredictionEngineObjectsInPool, maxPoolSize:= _maxPredictionEngineObjectsInPool, expirationTime:= _expirationTime)
		End Function

		Public Function Predict(dataSample As TData) As TPrediction
			'Get PredictionEngine object from the Object Pool
			Dim predictionEngine As PredictionEngine(Of TData, TPrediction) = _predictionEnginePool.GetObject()
